...
export const searchProjectsByUserId = async (userId: string, searchParams: SearchParams): Promise<Result<PaginatedResults<ProjectSearchDTO>>> => {
    const request = apiClient.get<PaginatedResults<ProjectSearchDTO>>(`projects/user/${userId}`, {
        params: searchParams,
    });
