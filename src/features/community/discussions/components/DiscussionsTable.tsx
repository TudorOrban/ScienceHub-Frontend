    ...
    return (
        <div className="w-full overflow-x-auto">
            <table className="custom-table">
                ...
                <tbody className="">
                    {data?.results && !isLoading && data?.results.map(discussion => (
                        <tr key={discussion.id}>
                            <td>
                                <Link href={constructFeatureURL(Feature.Discussion, discussion?.title, discussion?.user ? [discussion?.user] : [], [])} className="pseudo-link font-medium">
                                    {truncateString(discussion.title, 28)}
                                </Link>
                            </td>
                            <td>
                                <UsersAndCollaborationsUI users={discussion?.user ? [discussion?.user] : []} collaborations={[]} />
                            </td>
                            <td>
                            ...
                            <td className="flex items-center justify-center">
                                <div className="w-32 h-10">
                                    <StandardTag tag={{
                                        label: discussion.isPublic ? "Public" : "Private",
                                        value: discussion.isPublic ? "public" : "private",
                                        icon: discussion.isPublic ? faGlobe : faLock,
                                        iconColor: discussion.isPublic ? "rgb(22 163 74)" : "rgb(55 65 81)",
                                    }} />
                                </div>
                            </td>
                        </tr>
